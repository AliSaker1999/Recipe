...
const HomePage = () => {
  ...
  {loading ? (
    // Show skeletons and friendly message
    <>
      <div className="col-span-full flex flex-col items-center py-12">
        <span className="text-pink-500 text-xl font-bold mb-3 animate-bounce">
          🍳 Waking up the chef... 
        </span>
        <span className="text-gray-500 mb-8">Our server is warming up. Recipes will appear in a few seconds!</span>
      </div>
      {[...Array(6)].map((_, idx) => <SkeletonRecipeCard key={idx} />)}
    </>
  ) : (
    recipes.map((r) => (
      <div
